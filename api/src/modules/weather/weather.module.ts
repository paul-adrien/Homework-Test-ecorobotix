 ...
 *
 * Builds the provider registry (Open-Meteo + Yr.no), wraps each
 * cache-naive adapter with the LRU+TTL decorator, injects the registry
 * into the use cases, and registers the routes under `/api`.
 */
export async function registerWeatherModule(
  ...
  const getCurrentAndDailyUseCase = createGetCurrentAndDailyUseCase({ registry });
  const getHourlyUseCase = createGetHourlyUseCase({ registry });
